...
export const WalletProvider = ({ children }) => {
  ...
  const [ChakulaContract, setChakulaContract] = useState(null);
  ...
  const registerFarmer = async () => {
    if (ChakulaContract && account) {
      try {
        await ChakulaContract.methods.registerFarmer().send({ from: account });
  ...
  const getAllProduce = async () => {
    console.log(ChakulaContract, account);
    if (ChakulaContract && account) {
      try {
        console.log("Fetching produce...");
        const produceList = await ChakulaContract.methods.getAllProduce().call();
  ...
  const getProduceById = async (produceId) => {
    if (ChakulaContract && account) {
      try {
        const produce = await ChakulaContract.methods
  ...
  const payFarmer = async (requestId) => {
    if (ChakulaContract && account) {
      try {
        await ChakulaContract.methods.payFarmer(requestId).send({ from: account });
